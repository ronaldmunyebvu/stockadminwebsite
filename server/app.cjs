...
app.post('/api/auth/admin/signup', requireDatabase, async (req, res) => {
  const { shopName, fullName, identifier, email: legacyEmail, phone, password, logoUrl, tagline } = req.body
  ...
  try {
    ...
    if (existingOrg.rowCount) {
      ...
      await client.query('update organizations set logo_url=$1, tagline=$2 where id=$3', [logoUrl || null, tagline ? String(tagline).trim() : null, existingOrg.rows[0].id])
      organizationId = existingOrg.rows[0].id
    } else {
      const org = await client.query('insert into organizations (name, logo_url, tagline) values ($1, $2, $3) returning id', [shopName.trim(), logoUrl || null, tagline ? String(tagline).trim() : null])
...
app.get('/api/mobile/org', requireDatabase, auth, async (req, res) => { const result = await pool.query('select id,name,logo_url,tagline,variance_threshold_pct,variance_threshold_units,created_at from organizations where id=$1', [req.user.orgId]); res.json(result.rows[0]) })
