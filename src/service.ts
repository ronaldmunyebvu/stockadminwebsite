...
export async function createUser(orgId: string, input: { full_name: string; email: string; role: 'counter' | 'auditor'; phone?: string }): Promise<User> { return apiUrl ? request<User>('/api/admin/users', { method: 'POST', body: JSON.stringify(input) }) : { id: `demo-${Date.now()}`, org_id: orgId, ...input, is_active: true, setup_status: 'invited' as const, created_at: new Date().toISOString() } }
