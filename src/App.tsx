...
import { createExcelUpload, createItem, createItems, createLocation, createShopAdmin, createUser, createZone, deleteUser, isNeonConfigured, loadAdminData, scheduleStockCount, signInAdmin, signOutAdmin, updateThresholds, updateUserStatus } from './service'
...
export default function App() {
  ...
  const handlePrimaryAction = () => setDialog(page === 'team' ? 'team' : page === 'inventory' ? 'inventory' : page === 'sessions' ? 'threshold-confirm' : null)
    ...
    {dialog === 'threshold-confirm' && <ThresholdConfirmDialog onYes={() => { setDialog(null); setPage('settings') }} onNo={() => setDialog('schedule')} onClose={() => setDialog(null)} />}
  </div>
}
...
function Team({ data, setData }: { data: AdminData; setData: (data: AdminData) => void }) { const [error, setError] = useState(''); const toggle = async (id: string, value: boolean) => { try { await updateUserStatus(id, value); setData({ ...data, users: data.users.map(user => user.id === id ? { ...user, is_active: value } : user) }) } catch (err) { setError(err instanceof Error ? err.message : 'Failed to update user') } }; const remove = async (id: string) => { if (!window.confirm('Are you sure you want to permanently delete this team member? This cannot be undone.')) return; try { await deleteUser(id); setData({ ...data, users: data.users.filter(user => user.id !== id) }) } catch (err) { setError(err instanceof Error ? err.message : 'Failed to delete user') } }; return <div className="content-stack"><section className="team-summary"><div><span className="panel-kicker">People with access</span><h2>{data.users.filter(user => user.is_active).length} active teammates</h2><p>Manage who can count, review, and administer this workspace.</p></div><div className="avatar-stack">{data.users.filter(user => user.is_active).slice(0, 5).map(user => <div className="avatar" key={user.id} title={user.full_name}>{initials(user.full_name)}</div>)}</div></section><section className="panel table-panel"><div className="table-toolbar"><div><p className="panel-kicker">Access control</p><h3>Team members</h3></div><button className="filter-button"><SlidersHorizontal size={16} />Filter role</button></div>{error && <div className="auth-error" style={{ margin: '0 1.5rem' }}>{error}<button onClick={() => setError('')} style={{ marginLeft: 8, background: 'none', border: 'none', cursor: 'pointer', color: 'inherit' }}><X size={14} /></button></div>}<div className="table-scroll"><table><thead><tr><th>Member</th><th>Role</th><th>Status</th><th>Joined</th><th /></tr></thead><tbody>{data.users.map(user => <tr key={user.id}><td><div className="person-cell"><div className={`avatar avatar-small ${user.role === 'admin' ? 'avatar-olive' : ''}`}>{initials(user.full_name)}</div><span><strong>{user.full_name}</strong><small className="table-sub">{user.email}</small></span></div></td><td><span className="role-label">{roleLabels[user.role]}</span></td><td><button className={user.is_active ? 'toggle active' : 'toggle'} onClick={() => toggle(user.id, !user.is_active)}><span />{user.is_active ? 'Active' : 'Inactive'}</button></td><td className="muted">{formatDate(user.created_at)}</td><td>{user.role !== 'admin' && <button className="icon-button" title="Permanently delete member" onClick={() => remove(user.id)}><X size={16} /></button>}</td></tr>)}</tbody></table></div></section></div> }
...
function ThresholdConfirmDialog({ onYes, onNo, onClose }: { onYes: () => void; onNo: () => void; onClose: () => void }) {
  return <Dialog title="Set variance thresholds?" description="Before scheduling this count, would you like to configure variance thresholds in Settings?" onClose={onClose}>
    <div className="dialog-form">
      <p style={{ marginBottom: '1.25rem', color: 'var(--text-secondary, #6b7280)', lineHeight: 1.6 }}>Variance thresholds determine when a counted item is flagged for review. You can set a percentage threshold and a unit threshold. If you've already configured these, you can skip this step.</p>
      <div className="dialog-actions">
        <button className="button button-quiet" onClick={onNo}>No, continue scheduling</button>
