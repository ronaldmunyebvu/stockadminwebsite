...
function InviteDialog({ data, onClose, onCreated }: { data: AdminData; onClose: () => void; onCreated: (user: AdminData['users'][number]) => void }) {
  const [fullName, setFullName] = useState(''); const [email, setEmail] = useState(''); const [role, setRole] = useState<'counter' | 'auditor' | 'admin' | 'seller'>('counter'); const [busy, setBusy] = useState(false); const [error, setError] = useState('')
