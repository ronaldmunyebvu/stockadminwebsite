...
function Team({ data, setData }: { data: AdminData; setData: (data: AdminData) => void }) { const toggle = async (id: string, value: boolean) => { try { await updateUserStatus(id, value); setData({ ...data, users: data.users.map(user => user.id === id ? { ...user, is_active: value } : user) }) } catch { /* shared database errors are surfaced by the next refresh */ } }; const remove = async (id: string) => { if (!window.confirm('Are you sure you want to permanently remove this team member?')) return; try { await deleteUser(id); setData({ ...data, users: data.users.filter(user => user.id !== id) }) } catch { /* shared database errors are surfaced by the next refresh */ } }; return <div className="content-stack"><section className="team-summary"><div><span className="panel-kicker">People with access</span><h2>{data.users.filter(user => user.is_active).length} active teammates</h2><p>Manage who can count, review, and administer this workspace.</p></div><div className="avatar-stack">{data.users.filter(user => user.is_active).slice(0, 5).map(user => <div className="avatar" key={user.id} title={user.full_name}>{initials(user.full_name)}</div>)}</div></section><section className="panel table-panel"><div className="table-toolbar"><div><p className="panel-kicker">Access control</p><h3>Team members</h3></div><button className="filter-button"><SlidersHorizontal size={16} />Filter role</button></div><div className="table-scroll"><table><thead><tr><th>Member</th><th>Role</th><th>Status</th><th>Joined</th><th /></tr></thead><tbody>{data.users.map(user => <tr key={user.id}><td><div className="person-cell"><div className={`avatar avatar-small ${user.role === 'admin' ? 'avatar-olive' : ''}`}>{initials(user.full_name)}</div><span><strong>{user.full_name}</strong><small className="table-sub">{user.email}</small></span></div></td><td><span className="role-label">{roleLabels[user.role]}</span></td><td><button className={user.is_active ? 'toggle active' : 'toggle'} onClick={() => toggle(user.id, !user.is_active)}><span />{user.is_active ? 'Active' : 'Inactive'}</button></td><td className="muted">{formatDate(user.created_at)}</td><td><button className="icon-button" title="Remove member" onClick={() => remove(user.id)}><X size={16} /></button></td></tr>)}</tbody></table></div></section></div> }
