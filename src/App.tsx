...
import { approveSession, createExcelUpload, createItem, createItems, createLocation, createShopAdmin, createZone, createUser, deleteSession, deleteShop, deleteUser, getSales, getSessionEntries, getSessionReport, isNeonConfigured, loadAdminData, recountSession, rejectSession, scheduleStockCount, signInAdmin, signOutAdmin, submitReport, updateThresholds, updateUserStatus } from './service'
...
function SalesPage({ data }: { data: AdminData }) {
  ...
  const refresh = () => { setLoading(true); setError(''); getSales().then(setSummary).catch(err => setError(err instanceof Error ? err.message : 'Unable to load sales')).finally(() => setLoading(false)) }
  useEffect(() => { refresh() }, [])
...
function Inventory({ data, query, setQuery }: { data: AdminData; query: string; setQuery: (value: string) => void }) {
